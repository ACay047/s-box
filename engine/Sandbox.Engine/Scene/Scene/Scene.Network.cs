...
public partial class Scene : GameObject
{
	...
	/// </summary>
	internal void SerializeNetworkObjects( Connection source, List<object> collection, bool includeLocalObjects = false )
	{
		...
		foreach ( var target in networkedObjects )
		{
			...
			{
				AddWithAncestors( source, target, included, collection, includeLocalObjects );
			}
		}
	}
	...
	/// </summary>
	internal void AddWithAncestors( Connection source, NetworkObject target, HashSet<NetworkObject> included, List<object> collection, bool includeLocalObjects )
	{
		var current = target;
		while ( current is not null )
		{
			...
			collection.Add( current.GetCreateMessage( includeLocalObjects ) );
